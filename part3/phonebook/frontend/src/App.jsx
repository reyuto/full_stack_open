...
const App = () => {
  ...
  useEffect(() => {
    PersonAPI.getAll().then((results) => setPersons(results));
  }, []);
  ...
  const filteredPersons = persons?.filter(
    (person) =>
      person.name.toLocaleLowerCase().indexOf(filter.toLocaleLowerCase()) > -1,
  );
  ...
  const updatePersonNumber = (existingPerson, newNumber) => {
    ...
    PersonAPI.update(existingPerson.id, updatedPerson)
      .then(() => {
        setPersons((persons) =>
          persons.map((p) => (p.id === existingPerson.id ? updatedPerson : p)),
        );
        setMessage({
          text: `Number updated from ${existingPerson.number} to ${newNumber}`,
          type: "success",
        });
  ...
  const addNewPerson = (event) => {
    ...
    if (existingPerson) {
      if (existingPerson.number !== newNumber) {
        ...
        setMessage({
          text: `${newName} is already added to the phonebook`,
          type: "error",
        });
        setTimeout(() => {
          setMessage(null);
